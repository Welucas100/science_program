...
static void draw_wire(int I) {
    for (int y = 0; y <= HEIGHT + height_ + 1; ++y) {
        for (int x = 0; x <= WIDTH + width_ + 1; ++x) {
            if (y == height_ && x >= width_ - 1 && x <= WIDTH + 1) {
                gotoxy(x, y);
                textcolor(7, 7);
                printf(" ");
                textcolor(FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE, 0);
            }
            else if (y == HEIGHT / 2 && x >= width_ - 1&& x <= WIDTH  + 1) {
                gotoxy(x, y);
                textcolor(7, 7);
                printf(" ");
                textcolor(FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE, 0);
            }
            else if (x <= width_ && x >= width_ - 1&& y < HEIGHT / 2 && y > height_) {
                gotoxy(x, y);
                textcolor(7, 7);
                printf("  ");
                textcolor(FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE, 0);
            }
            else if (x >= WIDTH && x <= WIDTH+1 && y <  HEIGHT / 2 && y > height_) {
                gotoxy(x, y);
                textcolor(7, 7);
                printf("  ");
                textcolor(FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE, 0);
...
static void move_electrons(int elec_amount, int I, int NUM_ELECTRONS_X, int NUM_ELECTRONS_Y) {
    ...
    while (1) {
        ...
        // 세로축 전자 생성
        for (int i = 0; i < NUM_ELECTRONS_Y; ++i) {
            gotoxy(width_ - 1, electron_positions3[i]);
            printf("△ ");
        }
        for (int i = 0; i < NUM_ELECTRONS_Y; ++i) {
